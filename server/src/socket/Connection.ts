...
export default function Connection(socket: ServerSocket) {
    ...
    socket.on('outGoingMessage', (sender, address, message, callback) => {
        if (clients.has(address.e164)) {
            console.log('Có tin nhắn từ ' + sender.e164 + ' gửi đến ' + address.e164 + ' đang trực tuyến')
            ...
            console.log('Có tin nhắn từ ' + sender.e164 + ' gửi đến ' + address.e164 + ' đang ngoại tuyến')
